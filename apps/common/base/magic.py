        ...
        def wrapper(*args, **kwargs):
            ...
            for i in range(try_attempts):
                ...
                if status:
                    ...
                    logger.warning(
                        f"exec {func} failed. {try_attempts} times in total. now {sleep_time} later try " f"again...{i}"
                    )
            ...
            def wrapper(*args, **kwargs):
                ...
                if res:
                    while not res or res.get("status") != "ok":
                        time.sleep(0.5)
                        logger.warning(
                            f"exec {func} wait. data status is not ok. cache_time:{cache_time} "
                            f"cache_key:{cache_key}  cache data exist result:{res}"
                        )
                        res = cache.get(cache_key)
                with cache.lock(f"locker_{cache_key}", timeout=cache_time - invalid_time):
                    if res and n_time - res.get("c_time", n_time) < cache_time - invalid_time:
                        logger.debug(
                            f"exec {func} finished. cache_time:{cache_time} "
                            f"cache_key:{cache_key} cache data exist result:{res}"
                        )
                        return res["data"]
                    else:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"exec {func} finished. time:{time.time() - n_time} "
                                f"cache_time:{cache_time} cache_key:{cache_key} result:{res}"
                            )
                        except Exception as e:
                            logger.error(
                                f"exec {func} failed. time:{time.time() - n_time}  "
                                f"cache_time:{cache_time} cache_key:{cache_key} Exception:{e}"
                            )
