...
from apps.common.utils.request import get_browser, get_os, get_request_ident, get_request_ip
...
def check_is_block(username, ipaddr, ip_block=LoginIpBlockUtil, login_block=LoginBlockUtil):
    if ip_block and ip_block(ipaddr).is_block():
        ...
        raise APIException(
            _(
                "The address has been locked (please contact admin to unlock it or try" " again after {} minutes)"
            ).format(settings.SECURITY_LOGIN_IP_LIMIT_TIME)
        )
...
def save_login_log(request, login_type=UserLoginLog.LoginTypeChoices.USERNAME, status=True, channel_name=""):
    ...
    data = {
        ...
        "status": status,
        "agent": str(parse(request.headers["user-agent"])),
