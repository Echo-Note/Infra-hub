...
from apps.system.models import OperationLog, UserInfo, UserLoginLog
from apps.system.serializers.log import LoginLogSerializer


def trend_info(queryset, limit_day=30):
    today = timezone.now()
    limit_days = today - datetime.timedelta(
        days=limit_day, hours=today.hour, minutes=today.minute, seconds=today.second, microseconds=today.microsecond
    )
    data_count = (
        queryset.filter(created_time__gte=limit_days)
        .annotate(created_time_day=TruncDay("created_time"))
        .values("created_time_day")
        .annotate(count=Count("pk"))
        .order_by("-created_time_day")
    )
...
class DashboardViewSet(GenericViewSet):
    ...
    def user_active(self, request, *args, **kwargs):
        ...
        for date in active_date_list:
            x_day = today - datetime.timedelta(
                days=date - 1,
                hours=today.hour,
            ...
            x_day_active_user = (
                queryset.filter(last_login__gte=x_day)
                .values("last_login")
