...
class SecurityPasswordRuleSerializer(serializers.Serializer):
    ...
    SECURITY_ADMIN_USER_PASSWORD_MIN_LENGTH = serializers.IntegerField(
        min_value=6, max_value=30, required=True, label=_("Minimum length (Admin)")
    )
    SECURITY_PASSWORD_UPPER_CASE = serializers.BooleanField(required=False, label=_("Uppercase"))
    SECURITY_PASSWORD_LOWER_CASE = serializers.BooleanField(required=False, label=_("Lowercase"))
    SECURITY_PASSWORD_NUMBER = serializers.BooleanField(required=False, label=_("Digits"))
    SECURITY_PASSWORD_SPECIAL_CHAR = serializers.BooleanField(required=False, label=_("Special characters"))


login_ip_limit_time_help_text = _(
    "If the user has failed to log in for a limited number of times, " "no login is allowed during this time interval."
)
...
class SecurityLoginLimitSerializer(serializers.Serializer):
    ...
    SECURITY_LOGIN_LIMIT_COUNT = serializers.IntegerField(
        min_value=3, max_value=99999, label=_("User login failures count")
    )
...
class SecurityLoginAuthSerializer(serializers.Serializer):
    ...
    SECURITY_LOGIN_BY_SMS_ENABLED = serializers.BooleanField(
        required=False, default=True, label=_("Login by sms"), help_text=_("Enable send sms verify code to user")
    )

    SECURITY_LOGIN_BY_BASIC_ENABLED = serializers.BooleanField(
        required=False, default=True, label=_("Login by basic"), help_text=_("Enable basic verify to user login")
    )
...
class SecurityRegisterAuthSerializer(serializers.Serializer):
    ...
    SECURITY_REGISTER_TEMP_TOKEN_ENABLED = serializers.BooleanField(
        required=False,
        default=True,
        label=_("Register temp token"),
        help_text=_("Enable temporary tokens to prevent attacks"),
    )

    SECURITY_REGISTER_BY_EMAIL_ENABLED = serializers.BooleanField(
        required=False, default=True, label=_("Register by email"), help_text=_("Enable send email verify code to user")
    )
...
class SecurityResetPasswordAuthSerializer(serializers.Serializer):
    ...
    SECURITY_RESET_PASSWORD_CAPTCHA_ENABLED = serializers.BooleanField(
        required=False,
        default=True,
        label=_("Reset password captcha"),
    ...
    SECURITY_RESET_PASSWORD_ENCRYPTED_ENABLED = serializers.BooleanField(
        required=False,
        default=True,
        label=_("Reset password encrypted"),
        help_text=_("Enable encryption to prevent information leakage"),
    )
    ...
    SECURITY_RESET_PASSWORD_TEMP_TOKEN_ENABLED = serializers.BooleanField(
        required=False,
        default=True,
        label=_("Reset password temp token"),
        help_text=_("Enable temporary tokens to prevent attacks"),
    )
    ...
    SECURITY_RESET_PASSWORD_BY_EMAIL_ENABLED = serializers.BooleanField(
        required=False,
        default=True,
        label=_("Reset password by email"),
...
class SecurityVerifyCodeSerializer(serializers.Serializer):
    ...
    VERIFY_CODE_UPPER_CASE = serializers.BooleanField(required=False, label=_("Uppercase"))

    VERIFY_CODE_LOWER_CASE = serializers.BooleanField(required=False, label=_("Lowercase"))
...
class SecurityCaptchaCodeSerializer(serializers.Serializer):
    ...
    CAPTCHA_CHALLENGE_FUNCT = serializers.ChoiceField(
    ...
    CAPTCHA_LENGTH = serializers.IntegerField(
        default=4, min_value=2, max_value=16, label=_("Captcha code length"), help_text=_("Length of the captcha code")
    )
    ...
    CAPTCHA_TIMEOUT = serializers.IntegerField(
        min_value=1,
        max_value=60 * 24 * 7,
        label=_("Captcha timeout (minute)"),
    ...
    CAPTCHA_NOISE_FUNCTIONS = serializers.MultipleChoiceField(
        label=_("Noise functions"), default=NoiseFunctionsChoices.FUNCTION_NULL, choices=NoiseFunctionsChoices.choices
    )
